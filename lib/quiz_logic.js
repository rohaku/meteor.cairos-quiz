...
if(Meteor.isServer){
    Meteor.methods({
        "getDeveloperInfo" : function(){
            var _developerInof = {};
            _.find(_SERVER_URLS_, function(urlObj){
                if(process.env.SERVICE_URL === urlObj.URL){
                    _developerInof = urlObj;
                    FB_DEVELOPER_INFO = urlObj;
                    FBAppId = urlObj.FB_APP_ID;
                }
            });
            return _developerInof;
        }
    });
}
