...
if(Meteor.isClient){
    Meteor.startup(function () {
        ...
        UI.registerHelper('isFBLogin', function() {
            return Session.get("isFBLogin");
        });


        UI.registerHelper('setFBLoginStatus', function() {
            FB.getLoginStatus(function(response){

                console.log(response.status);
                if (response.status === 'connected') {
                    Session.set("isFBLogin", true);
                    //$("#rewardConnectContainer").remove();
                    //$("#rewardShareContainer").removeClass("hide");
                }else{
                    Session.set("isFBLogin", false);
                    //console.log($("#rewardConnectContainer").size());
